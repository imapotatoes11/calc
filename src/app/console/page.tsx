            ...
            <div className="bg-slate-50 flex flex-col gap-4 align-center content-center rounded-2xl shadow-2xl drop-shadow-2xl py-8 px-6 h-min">
                <div className="flex-col gap-4 flex justify-center">
                    <div className="w-96 flex justify-end flex-row gap-4">
                        <button className="mr-2" title="Clear History" onClick={() => setPastInputs([])}>
                            <DeleteIcon/>
                        </button>
                        <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)}
                               onKeyPress={handleKeyPress} placeholder="Enter an expression..."
                               className="font-mono bg-slate-100 p-4 rounded-md shadow-md focus:shadow-gray-400 focus:outline-none w-96 text-right font-medium mb-2 break-all"/>
                    </div>
                </div>
                <div className="absolute top-28">
                    ...
                    {pastInputs.length > 5 && (
                        <div onClick={handleOpenModal} className="w-96 flex justify-end cursor-pointer">
                            <div
                                className="font-mono bg-slate-100 p-4 rounded-md shadow-md focus:shadow-gray-400 focus:outline-none w-96 font-medium mb-2 break-all text-center">
                                ...
                            </div>
                        </div>
                    )}
                ...
                <div id="modal-content" style={{ maxHeight: '500px', overflowY: 'auto' }}>
                    {pastInputs.map((expression, index) => (
                        <div key={index} className="w-full flex justify-end mb-2">
                            <div className="font-mono bg-slate-100 p-4 rounded-md shadow-md focus:shadow-gray-400 focus:outline-none w-full text-right font-medium break-all">
                                {expression.value} = {expression.result}
                            </div>
                        </div>
                    ))}
